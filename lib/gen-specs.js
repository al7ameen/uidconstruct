...
<footer class="footer" role="contentinfo">
  <div class="container">
    ...
    <div class="footer-bottom">
      <span class="tagline">Values read from each site's own public stylesheets. No tracking pixels. No cookies.</span>
    </div>
  </div>
</footer>
<script>
...
// ------------------------------------------------------------------- spec page
function renderSpec(site, analysis, generatedAt) {
  ...
  ${counts.rest.length ? `<h3>Full ramp</h3>${swatchGrid(counts.rest, 'ramp')}` : ''}
    ...
    if (tokens.type.length) {
        sections.push(`<section class="spec-section container">
  <h2>Typography</h2>
  <p class="spec-note">Font families seen in the CSS: ${e.cssFonts && e.cssFonts.length ? esc(e.cssFonts.slice(0, 8).join(', ')) : 'none exposed as custom properties'}.</p>
  ${tokenTable(tokens.type, 'Typography tokens')}
</section>`);
    }
    if (tokens.geometry.length) {
        sections.push(`<section class="spec-section container">
...
function otherSitesBlock(current) {
    const others = SITES.filter((s) => s.slug !== current.slug).slice(0, 12);
...
async function main() {
    ...
    console.log(`\n${done.length} published this run, ${merged.length} total in hub, ${failed.length} skipped/failed.`);
    if (failed.length) console.log('FAILED: ' + failed.map((f) => `${f.slug} (${f.reason})`).join('; '));
}

if (require.main === module) main().catch((e) => { console.error('FATAL', e); process.exit(1); });
module.exports = { buildSpecText, parseTokenLines, renderIndex, renderSpec, splitColors };
